...
The front end can show an approval or external-execution UI. After the user acts, the current AgentScope Java recovery path is to call `runAgent` again with the real tool result message:
...
`RunAgentInput` currently has no official `resume` field, so do not rely on a `resume` field in documentation or frontend code.
