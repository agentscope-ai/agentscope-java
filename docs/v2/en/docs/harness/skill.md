...
So the agent always puts that path in the command, for example `execute(command="python3 <files-root>/scripts/foo.py")` — no path guessing, no per-source variations to remember. The optional `working_directory` parameter accepts only a path relative to the workspace root; absolute paths, `~`, and `..` are not allowed. Do not pass `<files-root>` to it; omit it when running skill scripts unless the command needs a workspace-relative working directory.
