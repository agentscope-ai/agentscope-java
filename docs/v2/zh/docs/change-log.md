...
| 2.0 中已删除 | 替代方案 |
|---|---|
| `.memory(Memory)` | `.stateStore(AgentStateStore)` —— 会话历史保存在 `AgentState.getContext()`；配置好的 `AgentStateStore` 在每次 `call()` 后自动 save/load，按该次调用 `RuntimeContext` 的 `(userId, sessionId)` 寻址 |
| `.statePersistence(StatePersistence)` | 同上 —— `AgentStateStore` 已接管持久化职责 |
| `.structuredOutputReminder(StructuredOutputReminder)` | 不再需要 —— structured output 现在由模型层原生支持（`Model.supportsNativeStructuredOutput()`），框架自动选择 native JSON schema 或 tool-choice 回退 |
...
| `io.agentscope.core.model.StructuredOutputReminder` | 不再需要 —— structured output 由模型层原生处理 |
| `io.agentscope.core.agent.StructuredOutputCapableAgent` | 已删除 —— structured output 能力内联到 `ReActAgent`，模型层原生支持 |
...
| `StatePersistence` | **删除** —— 由 `AgentStateStore` 抽象接管 |
| `ToolkitState` | 移到 `io.agentscope.core.state.legacy.ToolkitState`（仅兼容，新代码不要引用） |
...
| 已移除 | 替代方案 |
|---|---|
| `ReActAgent.getCurrentSessionId()` | 由 `RuntimeContext.getSessionId()` 在 `call()` 时传入 |
| `ReActAgent.getCurrentUserId()` | 由 `RuntimeContext.getUserId()` 在 `call()` 时传入 |
| `AgentBase(name, desc, checkRunning, hooks)` 构造器 | 改用 `AgentBase(name, desc, hooks)` —— `checkRunning` 不再需要，并发由 per-session 序列化保证 |
