...
- 写入日记账的 section header —— 无论是 flush（`## Memory Flush — <时间戳>`）还是内置 `memory_save` 工具（`## Memory Save — <时间戳>`）—— 都使用 JVM 默认时区的**带偏移** ISO-8601 时间戳，例如 `2026-09-10T16:05:32.3096574+08:00`（旧版本固定输出 `...Z`）。文件名日期与 header 时间戳来自同一时刻，且字符串仍可通过 `OffsetDateTime.parse(...).toInstant()` 还原为原 `Instant`，读取逻辑无需改动。
