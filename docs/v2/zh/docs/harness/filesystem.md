...
HarnessAgent agent = HarnessAgent.builder()
    ...
    .stateStore(new RedisAgentStateStore(jedis)) // 状态也要分布式
    .build();
```
...
| 实现 | 说明 |
|------|------|
| `RedisStore` | 基于 Jedis，适合低延迟高并发 |
| `JdbcStore` | 基于 JDBC，适合 MySQL / PostgreSQL / H2 |
...
| 模式 | 读写行为 |
|------|---------|
| 本机 | `OverlayFilesystem`：写落 workspace（上层），读先 workspace 后 project（下层） |
