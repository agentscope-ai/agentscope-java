...
下面的例子一次覆盖三件事：工作区驱动的人格、会话持久化（相同 `sessionId` 的第二轮记得第一轮）、对话压缩。
...
`.agentscope/workspace/AGENTS.md` 存在时就是 agent 的人格来源。第二轮能答出来，是因为同 `sessionId` 自动恢复了上一轮的状态。多聊几轮触发压缩后，提炼出来的事实会落到 `workspace/memory/YYYY-MM-DD.md`，再周期性合并到 `MEMORY.md`。进程重启、`sessionId` 不变，agent 依然记得这一切。
