...
- 能力查询（`getModelName`、`supportsNativeStructuredOutput`、`getContextWindowSize`）报告**主模型**——链的稳定身份——因此不会观察到其他并发调用的活跃候选；要查看某次调用实际由哪个候选服务，使用 `FailoverListener`（或 warn 日志）。因此候选应与主模型**能力兼容**（上下文窗口相同或更大、结构化输出支持一致）。builder 在构建时会就候选的已知能力差异提示警告：上下文窗口明确更小，或候选声明了主模型不具备的结构化输出支持。反向（候选只是不具备主模型的支持能力）不告警——`supportsNativeStructuredOutput()` 没有"未知"状态，默认 `false` 与"确实不支持"无法区分，在此告警会对合法的混合供应商链产生噪音；如果混用结构化输出支持不同的供应商，请自行核对所选模型的能力。
