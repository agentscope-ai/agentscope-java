...
> **深入了解**: 关于 Agent Skills 的完整介绍和架构设计,请参阅 [Claude Agent Skills 官方文档](https://platform.claude.com/docs/zh-CN/agents-and-tools/agent-skills/overview)。
...
| 阶段 | 加载时机 | 内容 | 令牌成本 |
|------|----------|------|----------|
| **阶段 1: 元数据** | 智能体初始化时 | `name` 和 `description` | 每个 Skill 约 100 tokens |
| **阶段 2: 指令** | AI 判断需要使用该 Skill 时 | SKILL.md 完整内容 | < 5k tokens |
| **阶段 3: 资源** | AI 按需访问 | references/、scripts/ 等资源文件 | 按实际使用计算 |
...
- `name`: 必需,唯一标识符,只能包含小写字母、数字、下划线
- `description`: 必需,清晰描述技能功能和使用场景,帮助 AI 判断何时使用该技能
...
// Skill 的元数据会自动注入到 System Prompt 中
...
注册后,AI 会在 System Prompt 中看到 Skill 的元数据,并在需要时自动使用。渐进式披露流程:

```text
用户提问 → AI 识别相关 Skill → AI 调用工具加载完整内容 → AI 根据指令执行任务
```
