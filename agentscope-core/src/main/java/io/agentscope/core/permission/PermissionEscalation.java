...
public final class PermissionEscalation {
    ...
    public static Outcome resolve(
        ...
        if (reason.length() > MAX_JUSTIFICATION_LENGTH) {
            ...
            // Step back one char when the cut would split a surrogate pair in half.
            int cut = MAX_JUSTIFICATION_LENGTH - 1;
            if (Character.isHighSurrogate(reason.charAt(cut - 1))) {
                cut--;
            }
            reason = reason.substring(0, cut) + "…";
        }
