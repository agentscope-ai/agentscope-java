 ...
 *
 * <p>Register an instance of this class in a {@link io.agentscope.core.tool.ContextStore} (via
 * {@link io.agentscope.core.tool.ToolExecutionContext} or {@code RuntimeContext}) to have its
 * entries automatically included as the {@code meta} field of every MCP
 * {@link io.modelcontextprotocol.spec.McpSchema.CallToolRequest} sent through {@link McpTool}.
 *
 ...
 *
 * <p>1) Via {@link io.agentscope.core.tool.ToolExecutionContext}:
 * <pre>{@code
 * ToolExecutionContext ctx = ToolExecutionContext.builder()
 *     .register(new McpMeta(Map.of("traceId", "abc-123", "userId", "u456")))
 *     .build();
 * }</pre>
 *
 * <p>2) Via {@code RuntimeContext}:
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>3) Merging multiple meta sources (later entries override earlier ones for the same key):
 * <pre>{@code
 ...
 *
 * @see McpTool
 * @see io.agentscope.core.tool.ToolExecutionContext
 */
