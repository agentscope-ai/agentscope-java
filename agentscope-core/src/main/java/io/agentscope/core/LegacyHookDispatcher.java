...
final class LegacyHookDispatcher {

    private final ReActAgent agent;

    LegacyHookDispatcher(ReActAgent agent) {
        this.agent = agent;
    }

    // ==================== Generic dispatch ====================

    <T extends HookEvent> Mono<T> fire(T event) {
    ...
    Mono<PreReasoningEvent> firePreReasoning(List<Msg> msgs, Msg systemMsg, String modelName) {
        PreReasoningEvent event = new PreReasoningEvent(agent, modelName, null, msgs);
        event.setSystemMessage(systemMsg);
        return fire(event);
    }

    Mono<PostReasoningEvent> firePostReasoning(Msg msg, String modelName) {
        return fire(new PostReasoningEvent(agent, modelName, null, msg));
    }

    Mono<Void> fireReasoningChunk(Msg chunkMsg, ReasoningContext context, String modelName) {
    ...
    Mono<List<ToolUseBlock>> firePreActing(List<ToolUseBlock> toolCalls, Toolkit toolkit) {
    ...
    Mono<PostActingEvent> firePostActing(
    ...
    Mono<Void> fireActingChunk(ToolUseBlock toolUse, ToolResultBlock chunk, Toolkit toolkit) {
    ...
    Mono<PreSummaryEvent> firePreSummary(
            List<Msg> msgs,
    ...
    Mono<PostSummaryEvent> firePostSummary(
