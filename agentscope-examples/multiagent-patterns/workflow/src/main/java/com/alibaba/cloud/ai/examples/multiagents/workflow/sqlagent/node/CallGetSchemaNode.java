...
public class CallGetSchemaNode implements NodeAction {
	...
	public Map<String, Object> apply(OverAllState state) throws Exception {
		...
		if (toolUses.isEmpty()) {
			return Map.of("messages", List.of(toAssistantMessage(response)), "llm_response", toAssistantMessage(response));
		}
	...
	private AssistantMessage toAssistantMessage(Msg msg, List<ToolUseBlock> toolUses) {
		...
		for (ToolUseBlock tu : toolUses) {
			String argsJson;
			try {
				argsJson = JSON.writeValueAsString(tu.getInput() != null ? tu.getInput() : Map.of());
			}
			catch (JsonProcessingException e) {
				argsJson = "{}";
			}
			toolCalls.add(new AssistantMessage.ToolCall(tu.getId(), "function", tu.getName(), argsJson));
		}
