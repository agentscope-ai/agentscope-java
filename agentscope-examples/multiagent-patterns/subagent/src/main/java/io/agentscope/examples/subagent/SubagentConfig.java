...
public class SubagentConfig {
	...
	@Bean
	public SubagentTools subagentTools(Model model,
			@Value("${subagent.workspace-path:${user.dir}}") String workspacePathStr) throws Exception {
		...
		Map<String, Object> defaultToolsByName = Map.of(
		...
		// Task tools: load codebase-explorer, web-researcher, general-purpose from classpath:agents/*.md
		Resource[] agentResources = new PathMatchingResourcePatternResolver().getResources("classpath:agents/*.md");
		TaskToolsBuilder builder = TaskToolsBuilder.builder()
				...
				.taskRepository(new io.agentscope.examples.subagent.tools.task.DefaultTaskRepository());
		...
		ReActAgent.Builder orchestratorBuilder = ReActAgent.builder()
				.name("tech-due-diligence-assistant")
				.description("Orchestrates technical due diligence by delegating to codebase-explorer, web-researcher, general-purpose, and dependency-analyzer sub-agents")
				.model(model)
	...
	public AgentScopeAgent dependencyAnalyzerAgent(Model model,
			@Value("${subagent.workspace-path:${user.dir}}") String workspacePathStr) {
		Path workspacePath = Paths.get(workspacePathStr).toAbsolutePath().normalize();
	...
	@Bean
	public CompiledGraph orchestratorGraph(AgentScopeAgent orchestratorAgent) throws GraphStateException {
		StateGraph graph = new StateGraph("subagent_orchestrator", () -> {
