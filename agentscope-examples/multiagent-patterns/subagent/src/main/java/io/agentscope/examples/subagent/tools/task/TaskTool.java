...
public class TaskTool {
	...
	@Tool(
			...
			description = """
					...
					""")
	public String task(
			@ToolParam(name = "description", description = "Short (3-5 word) summary of what the agent will do") String description,
			@ToolParam(name = "prompt", description = "The detailed task for the sub-agent to perform", required = true) String prompt,
			@ToolParam(name = "subagent_type", description = "The type of specialized agent to use", required = true) String subagentType,
			@ToolParam(name = "run_in_background", description = "Set to true to run asynchronously; use TaskOutput to retrieve results", required = false) Boolean runInBackground) {
