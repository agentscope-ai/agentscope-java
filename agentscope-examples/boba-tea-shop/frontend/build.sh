...
Build and dockerize the frontend Vue.js application.
...
OPTIONS:
    ...
    --skip-build                Skip npm build (use existing dist folder)
...
EXAMPLES:
    ...
    $0 -v 1.0.0                                 # Build with custom version
...
if [ "$SKIP_BUILD" = false ]; then
    log_info "=== Phase 1: Build Frontend ==="
    ...
    log_warning "Skipping frontend build (using existing dist folder)"
fi
