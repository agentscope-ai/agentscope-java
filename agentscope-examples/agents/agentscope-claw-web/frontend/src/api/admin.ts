...
export const getOverview = () => apiFetch<OverviewView>('/api/admin/overview');
...
export const listInstances = () => apiFetch<InstanceView[]>('/api/admin/instances');
...
export const listAdminSessions = (limit = 100) =>
  apiFetch<AdminSessionView[]>(`/api/admin/sessions?limit=${limit}`);
...
export const listChannels = () => apiFetch<ChannelView[]>('/api/admin/channels');
export const getChannel = (id: string) => apiFetch<ChannelView>(`/api/admin/channels/${encodeURIComponent(id)}`);
...
export function openLogStream(
  ...
  (async () => {
    try {
      const res = await fetch('/api/admin/debug/logs/stream', {
        headers: authHeaders(),
