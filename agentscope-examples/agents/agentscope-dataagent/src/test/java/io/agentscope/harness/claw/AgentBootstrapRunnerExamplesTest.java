...
/**
 * Documents and verifies the recommended {@link ClawBootstrap} wiring patterns.
 *
 * <p>The internal {@link io.agentscope.harness.claw.gateway.HarnessGateway} is created automatically by
 * the bootstrap. Users interact exclusively through {@link ClawBootstrap#chatUiChannel()}
 * and {@link ClawBootstrap#start(io.agentscope.harness.claw.channel.Channel...)}.
 */
class AgentBootstrapGatewayExamplesTest {
    ...
    /**
     * Build a single agent and interact through {@link ClawBootstrap#chatUiChannel()}.
     ...
     * {@link ClawBootstrap.Builder#channel}, then {@link ClawBootstrap#start()} (no-arg)
    ...
    /**
     * Demonstrates the ad-hoc {@link ClawBootstrap#start(io.agentscope.harness.claw.channel.Channel...)}
     ...
     * {@link ClawBootstrap#start()} starts it without any builder {@code .channel()} call.
     */
    @Test
    void singleAgent_chatuiFromFileConfig_autoCreated() throws Exception {
        ...
        ClawBootstrap agentBootstrap = ClawBootstrap.builder().cwd(tempDir).model(model).build();
     ...
     * ClawBootstrap#chatUiChannel()} instances — default routing hits the main agent.
     */
    @Test
