...
**Dev vs prod:** `InProcessEnvironmentWorker` (default on) is same-JVM convenience. Production sets
`builder.hands.in-process-worker=false` and runs `HandsWorkerMain`.
