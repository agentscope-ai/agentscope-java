...
写入 Agent 时，服务端会把 toolsets **派生**到工作区 `tools.json`；**版本快照**才是 Session 的权威来源。设计说明见 [API_REFACTOR.md](../API_REFACTOR.md)。
...
Skills / workspace / shares：`/api/agents/{id}/skills/*`、`…/workspace/*`、`…/shares`（见 [MANAGED_AGENTS_API.md](../MANAGED_AGENTS_API.md)）。
