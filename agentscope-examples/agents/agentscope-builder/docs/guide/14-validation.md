...
缺口（packages / limited networking）见 [SANDBOX_GAPS.md](../SANDBOX_GAPS.md)。

---

## 路径 C · self_hosted（生产 Hands：纯出站 Worker）

**目标**：关进程内 Worker；独立 `HandsWorkerMain` 执行外化工具并续跑。  
**不要**与 `sandbox`（E2B）混淆。
...
# 其余同生产：JWT / DB / DASHSCOPE_API_KEY …
...
协议细节：[08-hands-worker.md](08-hands-worker.md)、[events/worker.md](../events/worker.md)。剩余缺口：[SELF_HOSTED_GAPS.md](../SELF_HOSTED_GAPS.md)。
...
| 客户机执行 / NAT | `self_hosted` | `BUILDER_HANDS_IN_PROCESS_WORKER=false` | `HandsWorkerMain` |
| 开发同机 self_hosted | `self_hosted` | 默认 in-process=`true` | 无（**非生产**） |
...
| self_hosted 一直 idle / 无工具执行 | 仍开着 in-process 却以为在测独立 Worker；或 Worker 未启动 / key 错误 |
