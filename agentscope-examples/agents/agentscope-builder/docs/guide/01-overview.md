...
AgentScope Builder 提供与 Claude Managed Agents **同构**的产品面：**Agent / Environment / Session / Events**。你定义 Agent 与执行环境，通过 Session 发用户事件，由服务端托管 agent loop、工具编排、事件落库与 SSE；运行时基于 **AgentScope Harness**。
...
- 路由与缺口：[MANAGED_AGENTS_API.md](../MANAGED_AGENTS_API.md)
