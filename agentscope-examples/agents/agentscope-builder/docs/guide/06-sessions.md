...
| `GET` | `/api/sessions/{id}/events/stream` | SSE；可选重复 `event_deltas=` |
| `GET` | `/api/sessions/{id}/hands-stats` | Hands 租约指标 |

## 驱动运行

1. **发消息**：`user.message`，`payload.text`（也认 `message` / `content`）。
2. **中断**：`user.interrupt`。同 JVM 直接打断 Flux；跨副本可能 409（turn 在其他实例）。
...
产品集成请以本模块 HTTP 为准，勿依赖已弃用的 `/api/agents/{id}/chat/*`。
