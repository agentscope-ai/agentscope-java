...
Managed Environment `type=sandbox` 使用已有扩展 **`agentscope-extensions-sandbox-e2b`**（`E2bFilesystemSpec`）：Brain 向 E2B API 申请云端容器，shell/FS 在 E2B 内执行。**Builder 不再使用本机 Docker / Daytona。**
...
依赖与运行时请打进 **自定义 E2B template**；Claude 式 `packages` / `networking` **本迭代不强制执行**（见 [SANDBOX_GAPS.md](../SANDBOX_GAPS.md)）。
...
- 开发：默认 `builder.hands.in-process-worker=true`，同 JVM `InProcessEnvironmentWorker` 跑通事件驱动路径（**不是**生产形态）。
- 生产：`builder.hands.in-process-worker=false` + `HandsWorkerMain` + environment key；Worker 无入站端口、无共享盘要求。
