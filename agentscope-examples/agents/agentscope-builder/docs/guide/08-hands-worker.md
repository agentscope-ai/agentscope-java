...
| `self_hosted` + 独立 `HandsWorkerMain` | **生产**：客户基础设施上的纯出站执行器 |
| `self_hosted` + `InProcessEnvironmentWorker` | **开发便利**：同 JVM 事件驱动执行器，**不是**生产 self_hosted |
...
| 模式 | 配置 / 入口 |
|---|---|
| 默认同 JVM（开发） | `builder.hands.in-process-worker=true`，`InProcessEnvironmentWorker` 本地执行外化工具等价物 |
| 独立 Worker（生产） | `builder.hands.in-process-worker=false` + `io.agentscope.builder.worker.HandsWorkerMain` |
...
仍欠缺项（SPI、Files、E2E、reclaim 等）见专用清单：[SELF_HOSTED_GAPS.md](../SELF_HOSTED_GAPS.md)。
