...
## 方式二：`application.yml` 配置（仅限 web 模块）

在 `agentscope-claw-web` 模块中，若 `.agentscope/agentscope.json` **不存在**，`ClawConfig` 会根据 `application.yml` 的 `claw.agent.*` 属性**自动生成**一份单 Agent 配置。
