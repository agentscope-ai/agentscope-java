...
### 使用本地 OpenAI-compatible 模型运行

CLAW 示例也可以通过启用 `local-openai` profile，连接本地或私有化部署的
OpenAI-compatible 模型服务。

这个 profile 适用于 LM Studio、vLLM、Ollama、Xinference，或其他暴露
OpenAI-compatible chat completions API 的本地模型服务。
