...
> 面向后续 CLI / UI / Console / SDK 发布：把 AgentScope Service 的 HTTP 面按 **控制面（Control Plane）** 与 **数据面（Data Plane）** 整理，并对照 [Claude Managed Agents](https://platform.claude.com/docs/en/managed-agents/overview)（`/v1/*`，beta `managed-agents-2026-04-01`）。
>
...
| `POST` | `/api/deployments/webhook/{token}` | Webhook 触发 | AgentScope Service 扩展 |
...
| `GET` | `/api/sessions/{id}/hands-stats` | Hands 租约指标 | AgentScope Service 扩展 |
...
`/api/agents/{id}/chat/*`（旧 SSE 聊天）与 `/api/agents/{id}/sessions/*`（旧 gateway-session inbox / transcript）均已删除；唯一会话面为上文 managed `/api/sessions/*`。
...
Worker 认证：`X-Builder-Environment-Key`（create / rotate 时明文出现一次）。进程内 worker 已随四层拆分移除，`self_hosted` 一律由独立 `HandsWorkerMain`（service-scheduler jar）经这些 API 出站对接。
...
| **Agent 更新动词** | Claude 用 `POST` | AgentScope Service 用 `PUT` | 文档/SDK 约定即可；可选兼容别名 |
...
| 项 | 说明 |
|---|---|
| **稳定 OpenAPI / 版本头** | Claude 有 beta header；AgentScope Service 无对外 API 版本策略 |
...
| **Idempotency / 限流** | Claude 有 org RPM；AgentScope Service 未作为产品面承诺 |
