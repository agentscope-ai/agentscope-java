...
| 参考执行器（内置工具） | `HandsWorkerMain` + `LocalHandsToolExecutor`（service-scheduler） |
...
| **session 结束判定** | `HandsWorkerMain` 用「连续空闲 pending 轮次」启发式 `stop`，**未**查询 session `idle`/`terminated` 状态 API | **P2** |
...
| 项 | 说明 |
|---|---|
| **Worker 归属调度层** | 进程内 Worker 已随四层拆分移除；开发同机也跑独立 `HandsWorkerMain`（service-scheduler jar），勿再找 `builder.hands.in-process-worker` 开关 |
...
1. **跨机 E2E 测试**（独立 `HandsWorkerMain` 进程，断言执行面在 Worker 而非数据面）  
...
| 执行器 | `…/worker/HandsWorkerMain.java`、`…/selfhosted/LocalHandsToolExecutor.java`（service-scheduler） |
| 单测 | `SelfHostedHandsUnitTest.java`（service-scheduler）+ `SelfHostedHandsDataPlaneTest.java`（service-dataplane） |
