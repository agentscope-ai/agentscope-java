from probe import *
s=json.loads((ROOT/'state.json').read_text());keys=json.loads(pathlib.Path('/tmp/agentscope-it-20260907-1130-endpoint-keys.json').read_text());tag=sys.argv[1] if len(sys.argv)>1 else datetime.datetime.now().strftime('%H%M%S');summary={}
