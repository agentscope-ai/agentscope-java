from probe import *
s=json.loads((ROOT/'state.json').read_text());tag=sys.argv[1] if len(sys.argv)>1 else 'final';keys=json.loads(pathlib.Path('/tmp/agentscope-it-20260907-fix1-endpoint-keys.json').read_text());agentids=set(s['agents'].values())
