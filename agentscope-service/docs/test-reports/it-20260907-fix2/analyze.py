...
for rid,g in d['runs'].items():
 ...
 if run['state'] in runterminal:
  endings=[e for e in es if e['type']=='run.'+run['state']]
  if len(endings)!=1:findings.append({'kind':'missing_or_duplicate_run_terminal_event','runId':rid,'state':run['state'],'count':len(endings)})
 for node in g.get('nodes',[]):
  if node['runId']!=rid:findings.append({'kind':'wrong_node_run','nodeId':node['id']})
  if run['state'] in runterminal and node['state'] not in ['succeeded','failed','cancelled','skipped']:findings.append({'kind':'active_node_in_terminal_run','nodeId':node['id'],'state':node['state']})
 for taskSummary in tasks:
  t=d['tasks'][taskSummary['id']]
...
for sid,v in d['sessions'].items():
 ...
 for e in evs:
  if e.get('eventType')=='agent.tool_result' and e.get('toolName')=='web_search' and 'TAVILY_API_KEY is not set' in str(e.get('toolOutput')):
   meta=e.get('frameworkMeta') or {}
   linked=[re for ev in d['runEvents'].values() for re in ev.get('events',[]) if re['type']=='agent_tool.failed' and re.get('payload',{}).get('toolCallId')==meta.get('toolCallId')]
   if str(meta.get('state')).upper() not in ['ERROR','DENIED','INTERRUPTED'] or len(linked)!=1:findings.append({'kind':'web_error_not_projected_exactly_once','sessionRef':sid,'seq':e['seq'],'toolCallId':meta.get('toolCallId'),'state':meta.get('state'),'runFailures':len(linked)})
