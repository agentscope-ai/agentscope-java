...
def contains_marker(value, marker):
 if isinstance(value,str):
  if re.sub(r"\s+", "", marker) in re.sub(r"\s+", "", value): return True
  try: return contains_marker(json.loads(value), marker)
  except (ValueError,TypeError,RecursionError): return False
 if isinstance(value,dict):
  if '=' in marker:
   key,expected=marker.split('=',1)
   if key in value and str(value[key])==expected: return True
  return any(contains_marker(v,marker) for v in value.values())
 if isinstance(value,list): return any(contains_marker(v,marker) for v in value)
 return False
...
for name,marker in [('single-managed','MANAGED=400'),('single-qoder','QODER=apple,pear,zebra'),('single-codex','CODEX=prime')]:
 ...
 check(name+' 基础业务结果',r['state']=='succeeded' and contains_marker(delivery(g),marker),{'runId':r['id'],'state':r['state'],'expected':marker,'delivery':delivery(g)})
...
for label,cid,marker in [('mention A→B→A',cross,'CROSS_ACK=144'),('管理员更正需求',correct,'ADMIN_FINAL=156')]:
 ...
 check(label+' 内容',r['state']=='succeeded' and contains_marker(out,marker),{'runId':r['id'],'state':r['state'],'expected':marker,'delivery':out})
...
for name,marker in [('managed-job-ok','API_TOTAL=144'),('qoder-job-ok','API_WORDS=4'),('team-job-ok','TEAM_OK;CALC=400;SORT=apple,pear,zebra')]:
 g=graph('api-'+name);p=d['public'][name]['invocation']
 if name!='team-job-ok':check(name+' 内部业务',g['run']['state']=='succeeded' and contains_marker(delivery(g),marker),{'runId':g['run']['id'],'delivery':delivery(g)})
 check(name+' 公开 API 返回业务结果',p['status']=='completed' and contains_marker(p.get('result'),marker),{'invocationId':p['id'],'status':p['status'],'result':p.get('result')})
...
after=s['idempotencyAfterCompletion'];check('完成后相同请求幂等重试',after['status'] in [200,202] and after['response'].get('invocationId')==idem['first']['invocationId'],after)
...
replay=json.loads((ROOT/'evidence/sse-resume-managed-job.json').read_text());check('SSE 断点续读',replay['status']==200 and replay['ids']==[int(line[4:]) for line in json.loads((ROOT/'evidence/stream-managed-job-ok.json').read_text())['stream'].splitlines() if line.startswith('id: ') and int(line[4:])>4],{k:v for k,v in replay.items() if k!='stream'})
...
for name,marker in [('managed-job-ok','API_TOTAL=144'),('qoder-job-ok','API_WORDS=4'),('team-job-ok','TEAM_OK;CALC=400;SORT=apple,pear,zebra')]:
 data=[json.loads(line[6:]) for line in streams['stream-'+name]['stream'].splitlines() if line.startswith('data: ')]
 check(name+' SSE 返回业务结果',contains_marker(data,marker),{'expected':marker,'events':len(data)})
