...
for r in ex['runs']:
 g=api('/api/v1/orchestration-runs/'+r['id']+'/graph')
 if any(t['status'] not in ['completed','failed','cancelled'] for t in g['tasks']):
  print('cross mention tasks still active');sys.exit(0)
