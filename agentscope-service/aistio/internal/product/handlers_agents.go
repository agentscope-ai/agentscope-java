...
func (s *Server) listAgents(c *gin.Context) {
	...
	if err := s.db.Pool.QueryRow(c.Request.Context(),
		`SELECT COUNT(*) FROM agents a WHERE a.archived_at IS NULL AND `+agentVisibilityClause+
			` AND (NOT $2::boolean OR a.agent_id=ANY($3::text[]))`,
		owner, restricted, allowedIDs).Scan(&total); err != nil {
...
func (s *Server) getAgent(c *gin.Context) {
	owner := currentResourceOwner(c)
	agentID := c.Param("id")
	if a, err := s.loadAgent(c.Request.Context(), owner, agentID); err == nil {
		a.EffectiveTier = "EDIT"
		c.JSON(http.StatusOK, a.toJSON())
		return
	}
	// 非本人所有：仅当 owner 把该 Agent 分享给当前用户时可读。
	a, err := s.loadSharedAgent(c.Request.Context(), currentUserID(c), agentID)
	if err != nil {
