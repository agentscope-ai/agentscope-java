...
async function fixture(page: Page, kind = 'hosted-runtime', edit = true, linked = true) {
  ...
  await page.route('**/api/**', async route => {
    ...
    if (path.startsWith('/api/v1/agents/worker/versions/')) return json({ version: { version: definition.version, snapshot: { definitionFiles: { 'AGENTS.md': (!definition.workspaceBinding || definition.workspaceBinding.version === 1) ? 'Published one' : 'Published two', 'skills/review/SKILL.md': 'Review skill' } } } });
...
test('Private file edits stay draft until explicitly published', async ({ page }) => {
  ...
  await expect.poll(() => state.saves.length).toBe(1);
  await expect(page.getByText('Published definition files · Agent v2.', { exact: false })).toBeVisible();
  expect(state.errors).toEqual([]);
});
