...
export const listComments=async(issueId:string)=>{const response=await api.get<{items:Comment[];externalSync?:Record<string,NonNullable<Comment['externalSync']>>}>(`/api/v1/issues/${encodeURIComponent(issueId)}/comments`);return {...response,items:response.items.map(comment=>({...comment,externalSync:response.externalSync?.[comment.id]}))}};
