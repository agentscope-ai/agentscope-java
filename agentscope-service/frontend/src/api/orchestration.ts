...
export interface ExecutionAttempt { id:string;agentTaskId:string;runId:string;nodeId:string;agentId?:string;bindingId?:string;attempt:number;dispatchGeneration:number;backendKind:string;state:string;runtimeBinding?:unknown;hostId?:string;agentInstanceId?:string;sessionId?:string;turnId?:string;providerSessionId?:string;workspaceKey?:string;failureCode?:string;failureMessage?:string;usage?:unknown;createdAt:string;startedAt?:string;completedAt?:string }
export interface RuntimeBindingCandidate {binding:{agentId:string;bindingId:string;kind:'managed'|'external-application'|'hosted-runtime';ownerRef?:string;managedDefinitionRef?:string;instanceSelector?:Record<string,string>;runtimeProfileId?:string;runtimePoolId?:string};requiredCapabilities?:Record<string,unknown>;securityConstraints?:Record<string,unknown>}
export interface AgentRuntimePolicy {id?:string;tenant:string;namespace:string;agentId:string;candidates:RuntimeBindingCandidate[];selectionMode:'ordered';fallbackMode:'disabled'|'fresh';maxConcurrency?:number;queueTimeoutSeconds?:number;attemptTimeoutSeconds?:number;retryPolicy?:unknown;version?:number}
...
export const listRunEvents=(id:string)=>api.get<{events:RunEvent[]}>(`/api/v1/orchestration-runs/${id}/events`);
...
export const getRuntimePolicy=(tenant:string,namespace:string,agentId:string)=>api.get<{policy:AgentRuntimePolicy}>(`/api/v1/agent-runtime-policies/${encodeURIComponent(agentId)}${q({tenant,namespace})}`);
export const putRuntimePolicy=(agentId:string,body:AgentRuntimePolicy)=>api.put<{policy:AgentRuntimePolicy}>(`/api/v1/agent-runtime-policies/${encodeURIComponent(agentId)}`,body);
