...
export default function AgentCatalogDetailPage() {
  ...
  useEffect(() => {
    if (!agent.data || tabParam !== 'settings' || runtimeKind === 'managed') return;
    const nextParams = new URLSearchParams(params);
    nextParams.set('tab', 'runtime');
    setParams(nextParams, { replace: true });
  ...
  const instances = useQuery({ queryKey: ['catalog-agent-instances', agentId], queryFn: () => listCatalogAgentInstances(agentId), enabled: !!agentId, refetchInterval: ['overview', 'runtime'].includes(tab) ? 10_000 : false, refetchIntervalInBackground: false });
