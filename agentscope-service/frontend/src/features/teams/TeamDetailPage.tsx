...
export default function TeamDetailPage() {
  ...
  return <Page className="max-w-[1320px]">
    ...
    {tab === 'endpoints' && <PublishEndpointCard targetType="team" targetRef={team.id} targetName={team.name} />}

    {tab === 'activity' && <Card><CardHeader><CardTitle>Team work</CardTitle><CardDescription>Only AgentTasks created under this Team's frozen snapshots are shown; unrelated member traffic is excluded.</CardDescription></CardHeader><CardContent className="divide-y">{(tasks.data?.items ?? []).slice(0, 20).map(task => <Link key={task.id} to={scope.scopedPath(`/agent-center/activity/tasks/${task.id}`)} className="flex items-center justify-between gap-4 py-4 hover:bg-muted/30"><div><div className="font-medium">{task.teamRole || 'member'} · <AgentIdentity agentId={task.agentId} /></div><div className="mt-1 text-xs text-muted-foreground">{formatRelative(task.createdAt)} · {task.id.slice(0, 8)}</div></div><div className="flex items-center gap-2"><Badge tone={tone(task.status)}>{task.status}</Badge><ExternalLink className="h-4 w-4 text-muted-foreground" /></div></Link>)}{!tasks.isLoading && !(tasks.data?.items ?? []).length && <EmptyState title="No Team activity" description="Assign an Issue or invoke a published Team Endpoint to start an adaptive Run." />}</CardContent></Card>}
  </Page>;
}
