...
export default function AgentInstancesPage() {
  ...
  const items = useMemo(() => {
    ...
    return (instances.data?.items || []).filter((item) =>
      ...
      && (!needle || [agentNames.get(item.agentId), item.agentName, item.agentId, item.instanceKey, item.framework, item.backendKind, item.id].filter(Boolean).join(' ').toLowerCase().includes(needle)),
    );
  }, [agentNames, health, instances.data?.items, query]);
  ...
  return (
    <Page className="max-w-[1440px]">
      ...
      {!instances.isLoading && items.length === 0 ? <EmptyState title="No Agent instances" description="Connect an application through the Application SDK / ASDP or change the current filters." /> : <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{items.map((instance) => <Card key={instance.id}><CardHeader><div className="flex items-start justify-between gap-3"><div><CardTitle><Link className="hover:text-primary hover:underline" to={scope.scopedPath(`/agents/${instance.agentId}?tab=runtime`)}>{agentNames.get(instance.agentId) || instance.agentName || instance.agentId.slice(0, 8)}</Link></CardTitle><CardDescription>{instance.framework || instance.backendKind} {instance.frameworkVersion || ''}</CardDescription></div><Badge tone={healthTone(instance.health)}>{instance.health}</Badge></div></CardHeader><CardContent className="space-y-3 text-sm"><InstanceFact icon={Boxes} label="Instance" value={instance.instanceKey || instance.id} mono /><div className="grid grid-cols-2 gap-3"><InstanceFact icon={Users} label="Sessions" value={`${instance.activeSessions}/${instance.capacity}`} /><InstanceFact icon={Cpu} label="SDK" value={instance.sdkVersion || '—'} /></div><InstanceFact icon={Radio} label="Heartbeat" value={formatRelative(instance.lastSeenAt)} /></CardContent></Card>)}</div>}
    </Page>
  );
}
