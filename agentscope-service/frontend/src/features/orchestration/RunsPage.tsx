...
function RunDetail({ id }: { id: string }) {
  ...
  const identities = useEntityIdentities([
    ...
    ...(graph.data?.tasks || []).map(task => ({ type: 'issue', ref: String(task.issueId || '') })),
  ...
  const toolFailures = (events.data?.events || []).filter(event => event.type === 'agent_tool.failed');
  const firstToolFailure = toolFailures[0]?.payload as Record<string, unknown> | undefined;
  const failureSessionRef = firstToolFailure?.sessionRef ? String(firstToolFailure.sessionRef) : '';
  const queuedTaskIds = new Set(tasks.filter(task => task.status === 'queued').map(task => task.id));
  const dispatchDeferrals = (events.data?.events || []).filter(event =>
    event.type === 'agent-task.dispatch_deferred' && !!event.agentTaskId && queuedTaskIds.has(event.agentTaskId));
  const firstDispatchDeferral = dispatchDeferrals[0]?.payload as Record<string, unknown> | undefined;
  return <Page className="max-w-[1500px]">
    ...
    {toolFailures.length > 0 && <div className="mb-5 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-800"><div className="flex items-center justify-between gap-3"><div className="font-semibold">{toolFailures.length} tool failure{toolFailures.length === 1 ? '' : 's'} recorded</div>{failureSessionRef && <Link className="shrink-0 font-medium underline" to={scope.scopedPath(`/work/sessions/${failureSessionRef}`)}>Open agent session</Link>}</div><div className="mt-1 break-words text-xs">First failure: {String(firstToolFailure?.toolName || 'tool')} — {String(firstToolFailure?.message || 'No error message')}</div></div>}
    {dispatchDeferrals.length > 0 && <div className="mb-5 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900"><div className="font-semibold">{dispatchDeferrals.length} Agent task{dispatchDeferrals.length === 1 ? '' : 's'} waiting for runtime dispatch</div><div className="mt-1 break-words text-xs">{String(firstDispatchDeferral?.message || 'Runtime dispatch is temporarily unavailable. Automatic retry remains active.')}</div></div>}
    <div className="grid gap-5 xl:grid-cols-[2fr_1fr]">
      <section className="space-y-5">
        {run.input != null && <div className="rounded-xl border bg-white p-5"><h2 className="font-semibold">Original request input</h2><pre className="mt-3 whitespace-pre-wrap break-words rounded bg-muted p-3 text-sm">{typeof run.input === 'string' ? run.input : JSON.stringify(run.input, null, 2)}</pre></div>}
      ...
      <aside className="space-y-4">
        ...
        <div className="rounded-xl border bg-white p-4"><h2 className="font-semibold">Events</h2><p className="mt-1 text-xs text-muted-foreground">Provider-native events, dispatch diagnostics, and structured tool failures are retained on the same Run timeline.</p><ol className="mt-3 max-h-[40rem] space-y-2 overflow-auto">{events.data?.events.map(event => { const eventTask = tasks.find(task => task.id === event.agentTaskId); const failed = event.type === 'agent_tool.failed'; const deferred = event.type === 'agent-task.dispatch_deferred'; const recovered = event.type === 'agent-task.dispatch_recovered'; return <li key={event.id} className={failed ? "border-l-2 border-red-500 bg-red-50 p-2 text-xs text-red-800" : deferred ? "border-l-2 border-amber-500 bg-amber-50 p-2 text-xs text-amber-900" : recovered ? "border-l-2 border-emerald-500 bg-emerald-50 p-2 text-xs text-emerald-900" : "border-l-2 pl-3 text-xs"}><strong>#{event.sequence} {event.type}</strong><div className="mt-1 text-muted-foreground">{new Date(event.occurredAt).toLocaleString()}{eventTask && <> · <Link className="text-primary" to={scope.scopedPath(`/work/issues/${String(eventTask.issueId)}`)}><EntityIdentityText identities={identities} type="issue" entityRef={String(eventTask.issueId)} /></Link> · <Link className="text-primary" to={scope.scopedPath(`/work/executions/tasks/${eventTask.id}`)}>Task {eventTask.id.slice(0, 8)}</Link></>}{event.attemptId && <> · Attempt {event.attemptId.slice(0, 8)}</>}</div><div className={failed ? "text-red-700" : deferred ? "text-amber-800" : recovered ? "text-emerald-800" : "text-muted-foreground"}>{entityDisplayName(identities, event.actor.type, event.actor.ref)}</div>{event.payload != null && <details className="mt-1" open={failed || deferred || recovered}><summary className="cursor-pointer text-muted-foreground">Payload</summary><pre className="mt-1 overflow-auto whitespace-pre-wrap rounded bg-muted p-2">{JSON.stringify(event.payload, null, 2)}</pre></details>}</li>; })}</ol></div>
      </aside>
    </div>
  </Page>;
}
