...
function RunDetail({ id }: { id: string }) {
  ...
  const rerun = useMutation({ mutationFn: () => rerunRun(id), onSuccess: ({ run }) => navigate(scope.scopedPath(`/agent-center/activity/executions/${run.id}`)) });
  ...
  return <Page className="max-w-[1500px]">
    <Link to={scope.scopedPath('/agent-center/activity/executions')} className="text-sm text-muted-foreground">← Executions</Link>
