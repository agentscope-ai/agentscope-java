...
public class HarnessSkillMiddleware implements HarnessRuntimeMiddleware {
    ...
    public void prestageMarketplaceSkills(RuntimeContext ctx) {
        ...
        if (merged.isEmpty()) {
            stageEmptyRetainedIfSafe(ctx, merge.loadFailed());
            return;
        }
     ...
     * is removed when no skill survives merging/filtering. The sweep is confined to this
     * call's isolation scope and still honours
     * {@link MarketplaceStager#DEFAULT_ORPHAN_GRACE}: a directory another call staged within
     * the grace window is never mistaken for an orphan. Skipped when a repository failed to
     * load — an empty result caused by an error must not wipe the cache (outage protection).
     */
    private void stageEmptyRetainedIfSafe(RuntimeContext ctx, boolean loadFailed) {
        if (stager != null && !loadFailed) {
            stager.stage(List.of(), sourceNamespaces, scopeKeyFor(ctx));
        }
    }
