 ...
 *
 * <p>Persistence is scheduled only after normal stream completion. Cancellation intentionally
 * discards the in-progress turn, matching the previous stream composition behavior.
 */
public class TranscriptMiddleware implements HarnessRuntimeMiddleware {
    ...
    private void scheduleTranscript(Agent agent, RuntimeContext rc) {
        ...
        // defensive snapshot synchronously before detaching the transcript write from the
        // stream. Moving this list copy into the background task could capture the next turn or
        // miss messages from this one; only the transcript I/O is intentionally offloaded.
