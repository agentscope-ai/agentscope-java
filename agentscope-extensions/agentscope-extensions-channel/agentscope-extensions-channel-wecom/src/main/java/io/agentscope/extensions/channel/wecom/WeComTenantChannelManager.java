...
/**
 * Provides the {@link WeComChannel} of each tenant in a multi-tenant deployment, materializing it
 * on first use from {@link WeComCredentialResolver} results.
 *
 * <p>Wire one instance into {@link WeComCallbackController} and tenants can be added, rotated and
 * removed at runtime without the application constructing or registering a channel per tenant.
 *
 * <h2>Lifecycle</h2>
 *
 * <p>{@link #channelFor(String)} resolves the tenant's credentials on every call. The first call
 * materializes the tenant's channel; later calls return that same instance after refreshing its
 * credentials in place when they changed — see {@link WeComChannel#refreshCredentials}. A rotation
 * therefore never replaces the channel object: the tenant keeps its sessions, deduplication state
 * and bot-loop guard, and the credential fields that did not change keep their access-token cache.
 *
 ...
 * {@link WeComChannelRegistry} nor with a harness {@code ChannelManager}, so proactive outbound
 ...
 * lock-free. The lock is per tenant, so lookups for different tenants proceed independently, while a
 * lookup for one tenant waits out the resolver call of another lookup for the same tenant — keep
 * the resolver's own work to a lookup, not a remote round trip on the callback path.
 */
public final class WeComTenantChannelManager {

    private static final Logger log = LoggerFactory.getLogger(WeComTenantChannelManager.class);
    ...
    private final BiFunction<String, WeComChannelProperties, AccessTokenStore> tokenStoreFactory;
     ...
     * {@link #WeComTenantChannelManager(WeComCredentialResolver, ChannelConfig, Gateway,
     * InboundEventDeduplicator)} and {@link #WeComTenantChannelManager(WeComCredentialResolver,
     * ChannelConfig, Gateway, InboundEventDeduplicator, BiFunction)} to supply shared-storage
     * implementations instead.
     *
     * @param resolver resolves credentials per tenant key; consulted on every callback
     ...
     */
    public WeComTenantChannelManager(
            WeComCredentialResolver resolver, ChannelConfig routing, Gateway gateway) {
     ...
     *
     * <p>One deduplicator instance serves every tenant; the controller prefixes deduplication keys
     * with the tenant key, so a single bounded store (or a single shared backing store) covers the
     * whole deployment rather than one store per tenant.
     *
     * <p>Each tenant channel allocates a process-local access-token store per credential
     * generation; see {@link #WeComTenantChannelManager(WeComCredentialResolver, ChannelConfig,
     ...
     */
    public WeComTenantChannelManager(
            WeComCredentialResolver resolver,
    ...
    public WeComTenantChannelManager(
            ...
            Gateway gateway,
            InboundEventDeduplicator idempotency,
            BiFunction<String, WeComChannelProperties, AccessTokenStore> tokenStoreFactory) {
     ...
     */
    public Optional<WeComChannel> channelFor(String tenantKey) {
        Objects.requireNonNull(tenantKey, "tenantKey");
        WeComChannel channel = channels.get(tenantKey);
        if (channel == null) {
            ...
            Optional<WeComChannelProperties> initial = resolver.resolve(tenantKey);
            if (initial.isEmpty()) {
                return Optional.empty();
            }
            WeComChannelProperties seed = initial.get();
            channel = channels.computeIfAbsent(tenantKey, key -> materialize(key, seed));
        }
        synchronized (channel.credentialLock()) {
            Optional<WeComChannelProperties> resolved = resolver.resolve(tenantKey);
            if (resolved.isEmpty()) {
    ...
    public void evict(String tenantKey) {
        ...
        if (channels.remove(tenantKey) != null) {
            log.info("WeCom tenant '{}' evicted", tenantKey);
        }
    }

    private WeComChannel materialize(String tenantKey, WeComChannelProperties properties) {
        ...
        log.info("WeCom tenant '{}' channel materialized", tenantKey);
        return channel;
    }
}
