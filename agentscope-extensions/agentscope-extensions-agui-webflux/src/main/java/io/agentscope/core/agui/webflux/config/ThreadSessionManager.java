 ...
 *
 * <p>This manager maintains a pool of agent instances, each associated with a threadId.
 * When server-side memory is enabled, the same agent instance is reused for requests
 * with the same threadId, preserving conversation history across requests.
 *
 * <p><b>Usage:</b>
 * <pre>{@code
...
public class ThreadSessionManager {
     ...
     *
     * <p>This method is thread-safe. It uses atomic operations to ensure that
     * concurrent requests for the same threadId will share the same agent instance.
     *
