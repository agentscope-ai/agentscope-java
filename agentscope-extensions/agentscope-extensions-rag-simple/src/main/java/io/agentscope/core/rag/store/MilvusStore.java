...
public class MilvusStore implements VDBStoreBase, AutoCloseable {
    ...
    private static final String FIELD_TOTAL_CHUNKS = "total_chunks";
    ...
    private void addDocumentsToMilvus(List<Document> documents) {
        ...
        for (Document document : documents) {
            ...
            row.addProperty(FIELD_TOTAL_CHUNKS, metadata.getTotalChunks());
    ...
    private Document reconstructDocumentFromResult(SearchResp.SearchResult result, double score) {
        try {
            ...
            int chunkId = ((Number) entity.get(FIELD_CHUNK_ID)).intValue();
            int totalChunks = ((Number) entity.get(FIELD_TOTAL_CHUNKS)).intValue();
