...
/**
 * Configuration properties for AG-UI MVC integration.
 *
...
public class AguiMvcProperties {
    ...
    /**
     * Whether to manage conversation memory on the backend by threadId.
     * When enabled, the backend maintains agent instances per threadId,
     * preserving conversation history across requests.
     */
    private boolean serverSideMemory = false;

    /**
     * Maximum number of thread sessions to keep in memory.
     * Only used when serverSideMemory is enabled.
     */
    private int maxThreadSessions = 1000;

    /**
     * Session timeout in minutes. Sessions inactive for longer than this
     * will be removed. Set to 0 for no timeout.
     * Only used when serverSideMemory is enabled.
     */
    private int sessionTimeoutMinutes = 30;

    /**
     * HTTP header name to read agent ID from.
     * The agent ID can be passed via this header when making requests.
     */
    private String agentIdHeader = "X-Agent-Id";

    /**
     * Whether to enable path variable routing for agent ID.
     * When enabled, requests can be made to /agui/run/{agentId}.
     */
    private boolean enablePathRouting = true;

    /**
     * Timeout for SSE emitter in milliseconds.
     * This is the maximum time an SSE connection can stay open.
     */
